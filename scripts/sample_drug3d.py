...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_task",
        type=str,
        default="configs/sample/test/denovo_geom/base.yml",
    ...
    if args.config_model is not None:
        ...
        config_name = os.path.basename(args.config_task)[
            : os.path.basename(args.config_task).rfind(".")
        ]
    seed = (
        config.sample.seed
    )  # + np.sum([ord(s) for s in args.outdir]+[ord(s) for s in args.config_task])
    seed_all(seed)
    # config.sample.complete_seed = seed.item()
    # load ckpt and train config
    ckpt = torch.load(
        config.model.checkpoint, map_location=args.device, weights_only=False
    )
    ...
    if is_vscode:  # for debug using vscode
        ...
        names = (
            dir_names[is_sample + 1 :]
            + os.path.dirname(args.config_task).split("/")[1:]
        )
        log_root = "/".join([args.outdir.replace("outputs", "outputs_vscode")] + names)
        save_traj_prob = 1.0
        batch_size = 11
        num_mols = 100
        num_repeats = 2
    ...
    for samp_trans in config.get(
        "transforms", {}
    ).keys():  # overwirte transform config from sample.yml to train.yml
    ...
    num_workers = (
        train_config.train.num_workers if args.num_workers == -1 else args.num_workers
    )
    ...
    for i_repeat in range(num_repeats):
        ...
        if (
            ...
            in config.sample  # overwrite pos. and different for each repeat. for linker desigin with unknown frag pos
            or "overwrite_mol_repeat" in config.sample
        ):  # overwrite mol. for mol optimize round >= 1
            if "overwrite_pos_repeat" in config.sample:
                overwrite_pos_repeat = config.sample.overwrite_pos_repeat
                overwirter = OverwritePosRepeat(
                    config=overwrite_pos_repeat, i_repeat=i_repeat
                )
                transforms = Compose(
                    featurizer_list + [overwirter, task_trans] + addition_transforms
                )
            elif "overwrite_mol_repeat" in config.sample:
                overwrite_mol_repeat = config.sample.overwrite_mol_repeat
                overwirter = OverwriteMolRepeat(
                    config=overwrite_mol_repeat, i_repeat=i_repeat
                )
                transforms = Compose(
                    [overwirter] + featurizer_list + [task_trans] + addition_transforms
                )
            test_set = TestTaskDataset(
                data_cfg.dataset,
                config.task,
        ...
        for i_batch, batch in enumerate(test_loader):
            ...
            batch, outputs, trajs = sample_loop3(
                batch, model, noiser, args.device, is_ar=is_ar
            )
            ...
            for data_finished in mol_info_list:
                ...
                if len(save_output) > 0:
                    ...
                    torch.save(
                        output, os.path.join(sdf_dir, filename.replace(".sdf", ".pt"))
                    )
